...
impl OrtKoko {
    ...
    pub fn infer(
        ...
        if let Some(sess) = &self.sess {
            ...
            let input_names: Vec<String> = sess
                .inputs()
                .iter()
                .map(|input| input.name().to_string())
                .collect();

            let pick_name = |candidates: &[&str], fallback: &str| -> String {
                candidates
                    .iter()
                    .find_map(|cand| {
                        input_names
                            .iter()
                            .find(|name: &&String| name.as_str() == *cand)
                            .map(|name: &String| name.to_string())
                    })
                    .unwrap_or_else(|| fallback.to_string())
            };

            let token_name = pick_name(&["input_ids", "tokens", "input", "text"], "input_ids");
            let style_name = pick_name(&["style", "styles", "speaker_embedding"], "style");
            let speed_name = pick_name(&["speed", "speaking_rate", "rate"], "speed");

            let inputs: Vec<(Cow<str>, SessionInputValue)> = vec![
                (Cow::Owned(token_name), tokens_value),
                (Cow::Owned(style_name), style_value),
                (Cow::Owned(speed_name), speed_value),
            ];
